...
export function ProductsCarousel({ num }: { num: number }) {
  const plugin = React.useRef(Autoplay({ delay: num, stopOnInteraction: false }));
              ...
              <Link href="/" className="rounded-md">
                <Card className="border-none rounded-md  p-0 bg-transparent shadow-none">
                  <CardContent className="flex rounded-md  p-0 items-center justify-center border-none">
                    <Image src="/website.jpeg" alt="foto" className="w-full h-full lg: object-cover rounded-md" width={550} height={350} />
                  </CardContent>

                  <h1 className={` text-xl text-center my-5 capitalize`}> How to Stretch Your budget When Money Is Tight. </h1>
                </Card>
              </Link>
            </div>
          </CarouselItem>
